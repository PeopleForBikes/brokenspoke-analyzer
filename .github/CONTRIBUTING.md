...
In the sections below we provide instructions for how to set up a [developer environment locally](Developer_environment) or using a [development container](Development_container). Specific instructions will be described in other sections on this page.
...
Build and open the application in a development container in VS Code by running the
...
in the background and once the development container opens, a terminal will be available
to run the analyzer.
...
All the [administration tasks](Administration_tasks) can also be ran inside the development container.
